  ...
  <div class="container mt-5">
    ...
    <div class="list-group">
      <div v-for="discussion in filteredDiscussions" :key="discussion.id" class="list-group-item">
        ...
        <p class="discussion-ing"> Number of votes : {{ discussion.votes }}</p>
    ...
    <p v-if="filteredDiscussions.length === 0" class="text-center">No Events available.</p>
  </div>
</template>
...
export default {
  data() {
    return {
      ...
      topicOptions: [
        { value: null, text: 'All Events' }, // Option for all topics
  ...
  async mounted() {
    try {
      ...
      this.currentUserRole = user ? user.data().role : 'user';
    } catch (error) {
...
.discussion-ing {
  color: #a1181a; /* Medium dark color for content */
  font-weight: bold;
}
